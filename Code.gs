...
function asciiInitial(s){
  const ch = (String(s||'').trim()[0]||'').toUpperCase();
  if(!ch) return '';
...
function submitFromClient(p){
  try{
    ...
    if(SEC_STRICT && _serverNgCheck(ngTxt)){ return {ok:false, error:'ng word detected'}; }
...
function listRecords(q){
  try{
    ...
    const genre=q.genre||'', country=q.country||'', era=q.era||'', initial=q.initial||'';
    const limit = Number(q.limit||20), offset=Number(q.offset||0);
    const filtered = all.filter(r=>{
      if(genre && String(r.genre)!==genre) return false;
      if(country && String(r.country)!==country) return false;
      if(era){
        const de = decadeFromYearStr(r.release_year);
