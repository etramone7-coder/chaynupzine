...
(function validateConfig_(){
  var required = ['SPREADSHEET_ID','UPLOAD_FOLDER_ID'];
  required.forEach(function(key){
    if(!CONFIG[key]){
      throw new Error('必須プロパティが未設定です: ' + key +
        '（スクリプトのプロジェクト設定 > スクリプトプロパティ を確認してください）');
    }
  });
})();
/***** SHEET HEADERS ****************************************************/
...
function doGet(e) {
  ...
  else if (p === 'submit') page = 'submit'; // 追加
  // ▼ 追加：features 検索ページ
  else if (p === 'search_features' || p === 'features_search' || p === 'features.search') page = 'search_features';
  // ▼ 追加：lives / features の閲覧ページ
  else if (p === 'lives_browse' || p === 'lives.browse') page = 'lives_browse';
  else if (p === 'features_browse' || p === 'features.browse') page = 'features_browse';
...
function submitFromClient(payload){
  try{
    ...
    var sh = ss.getSheetByName(CONFIG.SHEET_RECORDS) || ss.insertSheet(CONFIG_SHEET_RECORDS);
...
/***** LIVES: submit *****************************************************/
// ▼▼ 変更点：複数アーティスト（artist, artist2..artist6）を結合保存し、
//            初期文字は先頭のアーティストで算出
function submitLive(payload){
  try{
    var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    var sh = ss.getSheetByName(CONFIG.SHEET_LIVES) || ss.insertSheet(CONFIG_SHEET_LIVES);
    ...
    // --- 複数アーティストを結合 ---
    var artists = [];
    var keys = ['artist','artist2','artist3','artist4','artist5','artist6'];
    for (var i = 0; i < keys.length; i++){
      var v = (payload[keys[i]] || '').trim();
      if (v) artists.push(v);
    }
    var artistCombined = artists.length ? artists.join(' / ') : (payload.artist || '').trim();
    var firstArtist = artists.length ? artists[0] : (payload.artist || '').trim();

    var bandSort    = normalizeBand_(firstArtist || '');
    var bandInitial = bandInitial_(bandSort);
    var editHash    = hashEditKey_(payload.edit_key || '');
    ...
    set('edit_key_hash', editHash);
    set('is_hidden',     false);
    set('genre', (payload.genre || '').trim()); // ← 追加：ジャンル保存
...
function submitFeature(payload){
  try{
    var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    var sh = ss.getSheetByName(CONFIG.SHEET_FEATURES) || ss.insertSheet(CONFIG_SHEET_FEATURES);
...
function editEntry(params) {
  try {
    ...
    if (action === 'unhide') {
      if (sheetName === CONFIG_SHEET_FEATURES && H['status']) {
    ...
    if (action === 'update') {
      ...
      if (sheetName === CONFIG_SHEET_LIVES) {
        ...
        // update時も複数アーティスト対応（artist/artist2..artist6 を結合）
        var upArtists = [];
        var klist = ['artist','artist2','artist3','artist4','artist5','artist6'];
        for (var ii=0; ii<klist.length; ii++){
          var vv = (p[klist[ii]] || '').trim();
          if (vv) upArtists.push(vv);
        }
        var upArtistCombined = upArtists.length ? upArtists.join(' / ') : (p.artist || '').trim();
        var upFirst = upArtists.length ? upArtists[0] : (p.artist || '').trim();
        var s2 = normalizeBand_(upFirst || '');

        set('artist', upArtistCombined);
        ...
        set('genre', (p.genre || '').trim()); // ← 追加：ジャンル更新
      }

      if (sheetName === CONFIG_SHEET_FEATURES) {
        set('title', (p.title || '').trim());
...
function submitReport(payload) {
  try {
    ...
    var ss  = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    var rep = ss.getSheetByName(CONFIG.SHEET_REPORTS) || ss.insertSheet(CONFIG_SHEET_REPORTS);
...
/***** SEARCH (Lives) ****************************************************/
// ▼▼ 変更点：artist 部分一致（先頭の "the/ザ" 等除去後）検索 + genre に対応
function listLives(params) {
  ...
  // 追加：アーティストキーワード（任意）— フロントからは params.artist or params.artist_kw で渡す
  var artist_kw  = String((params.artist_kw || params.artist || '')).trim();
  // 追加：ジャンル
  var genre      = String(params.genre || '').trim();
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    // アーティスト名 部分一致（the等の先頭除去後）
    if (artist_kw) {
      var hay = normalizeBand_(String(gv('artist') || '')).toLowerCase();
      var needle = normalizeBand_(artist_kw).toLowerCase();
    ...
    items.push({
      ...
      genre        : String(gv('genre') || '') // ← 追加：レスポンスにも含める
    });
  }
...
function listFeatures(params) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    items.push({
      ...
      genre      : String(gv('genre') || ''),
      era        : String(gv('era') || ''),
      region     : String(gv('region') || ''),                 // ★ 追加
      para1_text : String(gv('para1_text') || ''),             // ★ 追加
      para2_text : String(gv('para2_text') || ''),             // ★ 追加（任意）
      para3_text : String(gv('para3_text') || ''),             // ★ 追加（任意）
...
function getRecordByRow(rowIndex){
  try{
    ...
    // ★ ここだけ変更：EN欄が空でJPがある時にその場で翻訳・保存
    var descJa = String(gv('description')||'');
    var descEn = String(gv('description_en')||'');
    if (!descEn && descJa) {
      descEn = translateRecordDescription(r) || '';
    }
...
/** ★ 追加：レコードの紹介文を ja→en に翻訳して description_en に保存（空のときのみ） */
function translateRecordDescription(rowIndex){
  try{
    var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    var sh = ss.getSheetByName(CONFIG.SHEET_RECORDS);
    if (!sh) throw new Error('Records sheet missing');
    var H = buildHeaderIndex_(sh);

    var r = Number(rowIndex || 0);
    if (!(r >= 2 && r <= sh.getLastRow())) throw new Error('row out of range');

    var ja = String(sh.getRange(r, H['description']).getValue() || '');
    var en = String(sh.getRange(r, H['description_en']).getValue() || '');
    if (!ja) return '';
    if (en)  return en;

    var tx = LanguageApp.translate(ja, 'ja', 'en');
    sh.getRange(r, H['description_en']).setValue(tx);
    return tx;
  }catch(e){
    return '';
  }
...
function extractDriveId_(url){
  ...
  m = s.match(/uc\?export=(?:view|download)&id=([a-zA-Z0-9_-]+)/); if (m) return m[1];
...
function touchAuthOnce(){
  ...
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var shR = ss.getSheetByName(CONFIG.SHEET_RECORDS)  || ss.insertSheet(CONFIG_SHEET_RECORDS);
  var shL = ss.getSheetByName(CONFIG_SHEET_LIVES)    || ss.insertSheet(CONFIG_SHEET_LIVES);
  var shF = ss.getSheetByName(CONFIG_SHEET_FEATURES) || ss.insertSheet(CONFIG_SHEET_FEATURES);
  var shP = ss.getSheetByName(CONFIG_SHEET_REPORTS)  || ss.insertSheet(CONFIG_SHEET_REPORTS);
...
function pingRecords() {
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var sh = ss.getSheetByName(CONFIG_SHEET_RECORDS);
  if (!sh) return { ok: false, error: 'Records sheet missing' };
  var last = Math.max(1, sh.getLastColumn());
