...
function listRecords(q){
  try{
    ...
    const key = 'lv:list:v2:' + JSON.stringify({
...
function listLives(q){
  try{
    ...
    const items = out.slice(offset, offset+limit).map(r=>({
      ...
      venue:r.venue,       venue_en:r.venue_en,
region:r.region,     region_en:r.region_en,
prefecture:r.prefecture, prefecture_en:r.prefecture_en,
price:r.price,       price_en:r.price_en,
...
function editEntry(p){
  ...
  'artist',
  'title','title_en',
  'genre','date','open_time','start_time',
  'venue','venue_en',
  'region','region_en',
