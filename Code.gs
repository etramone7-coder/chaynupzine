...
function submitLive(p){
  try{
    ...
    // ★ reference_url の検証を緩和
    const refUrl = String(p.reference_url||'').trim();
    if(refUrl && refUrl!=="https://example.org/" && !_isAllowedHttpUrl(refUrl)) {
      return {ok:false, error:'invalid reference_url'};
    }
    ...
    const artistJoined = [p.artist,p.artist2,p.artist3,p.artist4,p.artist5,p.artist6]
                          .filter(Boolean).join(' / ');
    const row = {
      timestamp: new Date(),
      artist: artistJoined,
