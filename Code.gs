...
function doGet(e) {
  ...
  try{
    ...
    if (p.fn) {
      ...
      const map = {
        listRecords, getRecordByRow,
        listLives,
        listFeatures, getFeatureById
      };
      if (!map[fn]) {
        return ContentService.createTextOutput(JSON.stringify({ok:false,error:'fn not allowed via GET'}))
  ...
  // ★修正点: CSRF トークンを発行してフロントに埋め込み（client_id とペア化）
...
function doPost(e) {
  try {
    ...
    // ★修正: CSRF 検証を client_id とペアで（後方互換のため body.client_id or payload.client_id を参照）
    if (SEC_STRICT) {
      const clientId = String(body.client_id || (payload && payload.client_id) || '');
      const okCsrf = verifyCsrfToken_(String(body.csrf || ''), clientId);
      if (!okCsrf) {
...
function readAllCached(name){
  ...
  const key = 'sheetdump:'+name;
  const hit = cache.get(key);
  if (hit) { try{ return JSON.parse(hit); }catch(_){ /* ignore */ } }
  const rows = readAll(name);
  try{ cache.put(key, JSON.stringify(rows), 30); }catch(_){}
  return rows;
}
function _invalidateCache(name){
  try{ CacheService.getScriptCache().remove('sheetdump:'+name); }catch(_){}
}
...
function appendRowByHeader(name, obj){
  ...
  sh.appendRow(row);
  _invalidateCache(name); // ★追加: キャッシュ無効化
...
function _isAllowedHttpUrl(u){
  try{
    ...
    if (_rejectShortener(s)) return false; // ★追加: 短縮URL禁止
    const url = new URL(s);
...
// ★追加: 短縮URL判定
function _rejectShortener(u){
  return SHORTENERS_RE.test(String(u||''));
}
...
/* === CSRF（修正） === */
// ★修正: client_id とペアで保存/検証
function issueCsrfToken_(clientId){
  try{
    const cid = String(clientId||'').trim() || 'anon';
    const tok = Utilities.getUuid().replace(/-/g,'');
    CacheService.getScriptCache().put('csrf:'+cid+':'+tok, '1', 2 * 60 * 60); // 2h
    return tok;
  }catch(_){
    return Utilities.getUuid().replace(/-/g,'');
  }
}
function verifyCsrfToken_(tok, clientId){
  try{
    if(!tok) return false;
    const cid = String(clientId||'').trim() || 'anon';
    const v = CacheService.getScriptCache().get('csrf:'+cid+':'+String(tok));
    return !!v;
  }catch(_){ return false; }
}
...
function saveDataUrl(dataUrl, kindFolder){
  ...
  if(!m) return '';
  const mime = m[1];
  // ★変更: SEC_STRICT に関わらず常時検査
  if (ALLOWED_IMAGE_MIMES.indexOf(mime) < 0) {
  ...
  const ext = (mime.split('/')[1] || 'bin').replace(/[^a-z0-9.+-]/ig,'');
  const blob = Utilities.newBlob(bin, mime, 'image.'+ext); // ★修正: 拡張子付与
...
function submitFromClient(p){
  try{
    ...
    if (p.image_url && (!_isAllowedHttpUrl(p.image_url) || _rejectShortener(p.image_url))) {
    ...
    if (!descEn && p.description) {
      try { descEn = LanguageApp.translate(p.description, 'ja', 'en'); } catch(e) { descEn = ''; }
    }
...
function listRecords(q){
  try{
    const all = readAllCached(SHEETS.Records).filter(r => !String(r.is_hidden||'').match(/true/i)); // ★変更: キャッシュ
    // ▼追加：受け取ったクエリも trim
...
function submitLive(p){
  try{
    ...
    // 参考URL検証 / YouTube正規化
    const refUrl = p.reference_url||'';
    if(refUrl && (!_isAllowedHttpUrl(refUrl) || _rejectShortener(refUrl))) return {ok:false, error:'invalid reference_url'};
    const yt = _normalizeYouTube(p.youtube_url||'');
...
function listLives(q){
  try{
    const all = readAllCached(SHEETS.Lives).filter(r=>!String(r.is_hidden||'').match(/true/i)); // ★変更: キャッシュ
    let out = all;
    ...
    // ★修正: JST 固定で日付範囲
    const from = q.date_from? parseYmdJST(q.date_from) : null;
    const to   = q.date_to?   parseYmdJST(q.date_to)   : null;
    if(from || to){
      out = out.filter(r=>{
        const d = parseYmdJST(r.date||'');
        if(!d) return false;
    ...
    if(q.months && q.months.length){
      ...
      out = out.filter(r=>{
        const dd = parseYmdJST(r.date||''); if(!dd) return false;
...
function editEntry(p){
  try{
    ...
    // ★追加: 編集可能シートをホワイトリスト化
    const EDITABLE_SHEETS = new Set([SHEETS.Records, SHEETS.Lives, SHEETS.Features]);
    if (!EDITABLE_SHEETS.has(sheetName)) return {ok:false, error:'sheet not allowed'};
    ...
    const cur = {}; head.forEach((h,i)=>cur[h]=vals[i]);
    cur.is_hidden = hide;
...
function submitFeature(p){
  try{
    ...
    // 直URLでの画像指定がある場合はHTTPS/プライベート除外を検証 + 短縮URL禁止
    for (let i=1;i<=5;i++){
      for (let j=1;j<=3;j++){
        const kUrl = 'p'+i+'_img'+j+'_url';
        if (p[kUrl] && (!_isAllowedHttpUrl(p[kUrl]) || _rejectShortener(p[kUrl]))) {
          return {ok:false, error:'invalid image_url'};
        }
      }
    }
    ...
    // ★追加: slug 重複回避
    const baseSlug = p.slug || (slugify_(p.title)+'-'+(new Date().getTime().toString(36)));
    const slug = ensureUniqueSlug_(baseSlug);
...
// ★追加: slug の重複回避
function ensureUniqueSlug_(base){
  const all = readAllCached(SHEETS.Features);
  const used = new Set(all.map(r => String(r.slug||'')));
...
function listFeatures(q){
  try{
    const all = readAllCached(SHEETS.Features).filter(r => !String(r.is_hidden||'').match(/true/i)); // ★変更: キャッシュ
    let out = all.filter(r => String((r.status||'published')).toLowerCase()==='published');

    // ★追加: 受け取った条件があれば軽くサーバ側でも絞る（後方互換で任意）
    if (q) {
      if (q.genres && q.genres.length) {
        const set = new Set(q.genres.map(String));
        out = out.filter(r => set.has(String(r.genre||'')));
      }
      if (q.eras && q.eras.length) {
        const set = new Set(q.eras.map(String));
        out = out.filter(r => set.has(String(r.era||'')));
      }
      if (q.regions && q.regions.length) {
        const keys = q.regions.map(s => String(s||'').toLowerCase());
        out = out.filter(r => {
          const R = String(r.region||'').toLowerCase();
          return keys.some(k => k && R.indexOf(k)>=0);
        });
      }
    }

    const total = out.length;
    const limit = Number(q && q.limit || 100), offset=Number(q && q.offset || 0);
    const items = out.slice(offset, offset+limit).map(r=>{
      const d = {
...
function getFeatureById(p){
  try{
    const all = readAllCached(SHEETS.Features); // ★変更: キャッシュ
...
function submitReport(p){
  try{
    ...
    // ★修正: 同一 client_id の重複通報はユニークカウント
    const all = readAllCached(SHEETS.Reports)
      .filter(r => String(r.sheetName)===sheetName && Number(r.rowIndex)===rowIndex);
    const uniqClients = new Set(all.map(r => String(r.client_id||'')));
    let autoHidden = false;
    if (uniqClients.size >= REPORT_AUTOHIDE_THRESHOLD){
