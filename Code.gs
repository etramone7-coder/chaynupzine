...
function listRecords(q){
  try{
    // ★高速化：簡易キャッシュ（パラメータでキー化）
    const key = 'rc:list:' + JSON.stringify({
    ...
    // ★重要：browseでの言語切替用に description / description_en を返す
    const items = filtered.slice(offset, offset+limit).map(r=>({
      rowIndex:r.rowIndex,
      band:r.band, track:r.track, country:r.country, genre:r.genre,
...
function listLives(q){
  try{
    // ★高速化：簡易キャッシュ（パラメータでキー化）
    const key = 'lv:list:' + JSON.stringify({
    ...
    const items = out.slice(offset, offset+limit).map(r=>({
      rowIndex:r.rowIndex, artist:r.artist, title:r.title, title_en:r.title_en, genre:r.genre,
      date:r.date, open_time:r.open_time, start_time:r.start_time,
      venue:r.venue, region:r.region, prefecture:r.prefecture,
      price:r.price, description:r.description, description_en:r.description_en,
      // ★変更1：ここで都度正規化して返す（既存データが file/d/... でも表示される）
      image_url: normalizeDriveUrl_(r.image_url),
      youtube_url:r.youtube_url, status:r.status
    }));
  ...
  try{
    ...
    if (action === 'update'){
      ...
      if (sheetName === SHEETS.Records){
        allow = ['genre','country','band','track','release_year','catalog_no','format','description','description_en','image_url','youtube_url','author','is_hidden'];
        ...
        allow = ['artist','title','title_en','genre','date','open_time','start_time','venue','region','prefecture','price','description','description_en','image_url','youtube_url','author','status','is_hidden'];
        if (patch.image_url) patch.image_url = normalizeDriveUrl_(patch.image_url);
        ...
        allow = ['status','title','title_en','author','genre','genre_other','era','region','slug',
        ...
        try{
          ...
          for (let i=1;i<=5;i++){
            const jp = patch['para'+i+'_text'];
            const en = patch['para'+i+'_text_en'];
            if (jp && !en){
              patch['para'+i+'_text_en'] = LanguageApp.translate(String(jp),'ja','en');
            }else if (!jp && en){
              patch['para'+i+'_text'] = LanguageApp.translate(String(en),'en','ja');
            }
          }
        }catch(_){}
      } else {
...
function listFeatures(q){
  try{
    ...
    const items = out.slice(offset, offset+limit).map(r=>{
      const d = {
        rowIndex:r.rowIndex, slug:r.slug, status:r.status, title:r.title, title_en:r.title_en,
      ...
      d.para1_text_en = r.para1_text_en||''; d.para2_text_en=r.para2_text_en||''; d.para3_text_en=r.para3_text_en||'';
      return d;
    });
...
/**
 * Recordsシートで、description_en が空で description がある行だけ英訳して埋める。
 * 実行方法：エディタ上部の関数プルダウンから backfillDescriptionEn を選び「実行」
 * 事前に：権限承認ダイアログが出たら許可すること。
 */
function backfillDescriptionEn(){
  const SHEET = SHEETS.Records || 'Records';
  const SRC_LANG = 'ja';
  const DST_LANG = 'en';
  const DRY_RUN = false;       // ← true にすると書き込みせずログだけ
  const MAX_PER_RUN = 300;     // ← 1回で処理する最大行数（大きすぎると実行時間超過の恐れ）
  ...
  // ヘッダーの列位置取得
  const head = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0];
  const colMap = {}; head.forEach((h,i)=>colMap[String(h||'')] = i);
  ...
  // まとめて読み込み → 必要分のみ翻訳 → バッチ書き込み
  const vals = sh.getRange(2,1,lastRow-2+1, head.length).getValues();
  let updates = 0;
  let translated = 0;

  for (let r=0; r<vals.length; r++){
    const row = vals[r];
    const ja = String(row[ciDesc]||'').trim();
    const en = String(row[ciDescEn]||'').trim();

    // すでに英語が入っている／日本語が空 → スキップ
    if (en || !ja) continue;

    // 実際に翻訳
    let out = '';
    try{
      out = LanguageApp.translate(ja, SRC_LANG, DST_LANG);
    }catch(e){
...
/**
 * まずは挙動確認だけしたい時はこちらを実行（書き込み無しで対象件数だけ出力）
 */
...
/* Lives ヘッダー復元（reference_url を廃止し、英語列を追加） */
function restoreLivesHeader() {
  ...
  const HEADERS = [
    ...
    'venue','region','prefecture','price','description','description_en',
    'image_url','youtube_url','author','status','edit_key','client_id','is_hidden'
  ];
...
/* ★追加1：既存 Lives.image_url を一括正規化する補助関数（任意実行） */
...
/* ★追加2：不足ヘッダーだけを末尾に追加（既存の列は一切壊さない） */
...
/* ★追加4：ワンクリック移行（実行するだけでOK） */
function migrateLivesOnce(){
  ensureLivesExtraHeaders();
  normalizeLivesImageUrls();
  backfillLivesEn();
}
/* ★追加：Lives.description_en 列を追加し、必要なら翻訳で埋める */
function ensureLivesDescriptionEn(){
  const sh = getSheet(SHEETS.Lives);
  if(!sh) throw new Error('Lives sheet not found');
  ...
  const head = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0].map(String);
  let col = sh.getLastColumn();
  if (head.indexOf('description_en') < 0){
    col += 1;
    sh.getRange(1,col).setValue('description_en');
  }

  // 2. description_en が空で description がある行を翻訳
  const newHead = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0].map(String);
  const idx = {};
  newHead.forEach((h,i)=>{ idx[h]=i; });

  const last = sh.getLastRow();
  if (last < 2) return;

  const vals = sh.getRange(2,1,last-1,newHead.length).getValues();
  let touched = 0;
  for (let r=0; r<vals.length; r++){
    const row = vals[r];
    const descJa = String(row[idx['description']]||'').trim();
    const descEn = String(row[idx['description_en']]||'').trim();
    if (descJa && !descEn){
      try{
        row[idx['description_en']] = LanguageApp.translate(descJa,'ja','en');
        touched++;
      }catch(e){
        Logger.log('翻訳失敗 row='+(r+2)+': '+e);
      }
  ...
  if (touched){
    sh.getRange(2,1,vals.length,newHead.length).setValues(vals);
  }
  Logger.log('Lives.description_en 補完件数='+touched);
}
function migrateLivesData() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const sh = ss.getSheetByName("Lives");
  if (!sh) return;
  ...
  const HEADERS = [
    'timestamp','artist','title','title_en','genre','date','open_time','start_time',
    'venue','region','prefecture','price',
    'description','description_en',
    'image_url','youtube_url',
    'author','status','edit_key','client_id','is_hidden'
  ];
  ...
  // 旧ヘッダーを読む
  const oldHead = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0];
  const vals = sh.getRange(2,1,lastRow-1,oldHead.length).getValues();

  // 新しい順に並び替える
  const newVals = vals.map(row => HEADERS.map(h => {
    const idx = oldHead.indexOf(h);
    return idx >= 0 ? row[idx] : "";
  }));

  // シートをクリアして新しいヘッダーとデータを入れる
  sh.clear();
  sh.getRange(1,1,1,HEADERS.length).setValues([HEADERS]);
  sh.getRange(2,1,newVals.length,HEADERS.length).setValues(newVals);
}
