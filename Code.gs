...
function submitLive(p){
  try{
    ...
    // ★row 定義（image_url / youtube_url / status などを含む）
    const row = {
      timestamp: new Date(),
      artist: artistJoined,
      title: titleJa,
      ...
      description: descJa,
      description_en: descEn,
      image_url: (p.image_data ? saveDataUrl(p.image_data, 'Lives') : (p.image_url ? normalizeDriveUrl_(p.image_url) : '')),
      youtube_url: yt,
      author: p.author||'',
